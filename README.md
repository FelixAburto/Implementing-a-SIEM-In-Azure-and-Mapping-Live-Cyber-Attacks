...
<br />
The first thing to do is create a honey pot virtual machine in Azure and configure its firewall to allow all everything into the virtual machine:
...
<br />
Once the VM is created remote into the machine using Remote Desktop Connection. Once you are logged in you're going to turn off the firewall by typing "wf.msc" into the search bar, click on "Windows Defender Firewall Properties" and set the firewall state to "Off" in the Domain Profile, Private Profile, and Public Profile". After that you can comfirm that you are able to communicate to the VM from the internet by pinging it from your host machine:
...
The next step is to enable the ability to gather logs from the honey pot. To do this we need to go to Microsoft Defender Cloud. Once there click on "Environment Settings" and click on "Expand All". You should see the LAW that you created for your honey pot. Click on your LAW and enable "Foundational CSPM" and "Severs". Next click on "Data Collection" on the left side of your screen and enable "All Events":  
...
After this go back to the Log Analytics Workspace that you created and connect your honey pot to it. To do this you're going to click on "Virutual Machines" on the left side of your screen. Click on the VM that you made and click "Connect" :  
...
Next we will create a custom log within our Log Analytics Workspace. This will allows us to bring over the failed RDP log file that the script created into the LAW which will allow us to connected it into the Microsoft Sentinel. To create our custom logs we'll need to go to "Tables" under the LAW that we created. Click on create and select "New custom log (MMA-based)". Next it will ask you to input a sample log file and you will use the log file that the PowerShell script has created inside "C:\ProgramData\". You can do this by copying the text from the failed_rdp.log file and pasting it to a txt file on your host computer. From there simply select the txt file that you just created and click next until you are at the "Collection Path" screen. Here you will input the "C:\ProgramData\failed_rdp.log" file path which is where the log exists within the VM. Next we are going to enter in the custom log name (this can be whatever you want) and finally we're going review and create our custom logs.
<br />
...
<br />
Afterwards we need to make sure our custom log is working properly. To do this we are going to select "Logs" within our LAW. This should open up a section where we can input the name of your custom log and run it. If the custom log is running properly it should look like this:
<br />
...
<br />
The next step will be to create a workbook within the Microsoft Sentinel that we set up earlier. To do this we are going to go to our Sentinal that we created and on the next page click on "Workbook" under Threat Management. Afterwards click on "Add Workbook" and on the next page delete the default items by clicking "Edit". You should see three dots appear next to the items. Click on the the threed dots and click remove for each of them until there is nothing left. After the items are gone you should see the option to add new items. Click on that and select "Add Query" from the drop down menu. 
<br />
...
<br />
Next we will add KQL Regex Query to extract relevant information needed to map out the cyber attacks within Microsoft Sentinel. The query is included in a txt file within the repository. After you enter in the query run it to make sure it is extracting the data correctly. It should look like this afterwards:
<br />
